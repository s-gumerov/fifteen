...
describe('transformSecondsCountToWordExpression function', () => {
  ...
  test('equal one hour', () => {
    expect(transformSecondsCountToWordExpression(3600)).toBe(
      '1 час 0 мин 0 сек'
    )
  })
  test('more than one hour', () => {
    expect(transformSecondsCountToWordExpression(3700)).toBe(
      '1 час 1 мин 40 сек'
    )
  })
  test('more than two hours', () => {
    expect(transformSecondsCountToWordExpression(10000)).toBe(
      '2 час 46 мин 40 сек'
    )
  })
})
