...
export class CanvasController {
    ...
    drawField(
        fieldRef: React.RefObject<HTMLCanvasElement>,
        board: TBoard,
        ...
        for (let i = 0; i < gameFieldSize; i++) {
            for (let j = 0; j < gameFieldSize; j++) {
                ...
                if (board[i][j] === emptyCellValue) {
                    ctx.clearRect(
                        tileBorder + j * tileSize,
                        tileBorder + i * tileSize,
                        tileSize,
                        tileSize
                    )
                } else {
    ...
    getBoardAfterClick(event: React.MouseEvent, board: TBoard): TBoard {
        const yMouse =
            ((event.pageX - (event.target as HTMLCanvasElement).offsetLeft) /
                tileSize) |
            0
        const xMouse =
            ((event.pageY - (event.target as HTMLCanvasElement).offsetTop) /
                tileSize) |
            0
        ...
        if (emptyCell) {
            ...
            if (
                xEmptyCell === xMouse &&
                (yEmptyCell === yMouse - 1 || yEmptyCell === yMouse + 1)
            ) {
            ...
            if (
                yEmptyCell === yMouse &&
                (xEmptyCell === xMouse - 1 || xEmptyCell === xMouse + 1)
            ) {
                board[xEmptyCell][yEmptyCell] = board[xMouse][yMouse]
                board[xMouse][yMouse] = emptyCellValue
            }
    ...
    drawButton = (
        ...
        ctx.beginPath()
        ctx.moveTo(
            btnName.x + btnName.width - btnPathIndent,
            btnName.y + btnName.height
        )
        ctx.arcTo(
            btnName.x,
            btnName.y + btnName.height,
        ...
        ctx.fillText(
            btnName.text,
            (btnName.x + btnName.width + btnName.x) / 2,
            (btnName.y + btnName.y + btnName.height) / 2
        )
    }
